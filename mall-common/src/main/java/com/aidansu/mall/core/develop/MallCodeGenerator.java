package com.aidansu.mall.core.develop;
...
public class MallCodeGenerator {
	 ...
	 * 代码生成的包名
	 */
	private String packageName = "com.aidansu.mall";
	/**
	 * 代码后端生成的地址
	 */
	 ...
	 * 需要去掉的表前缀
	 */
	private String[] tablePrefix = {"mall_"};
	/**
	 * 需要生成的表名(两者只能取其一)
	 */
	private String[] includeTables = {"mall_test"};
	/**
	 * 需要排除的表名(两者只能取其一)
	 */
	...
	public void run() {
		...
		if (hasSuperEntity) {
			strategy.setSuperEntityClass("com.aidansu.mall.core.mybatis.base.BaseEntity");
			strategy.setSuperEntityColumns(superEntityColumns);
			strategy.setSuperServiceClass("com.aidansu.mall.core.mybatis.base.BaseService");
			strategy.setSuperServiceImplClass("com.aidansu.mall.core.mybatis.base.BaseServiceImpl");
		} else {
	...
	private InjectionConfig getInjectionConfig() {
		...
		InjectionConfig cfg = new InjectionConfig() {
			@Override
			public void initMap() {
				map.put("codeName", MallCodeGenerator.this.codeName);
				map.put("serviceName", MallCodeGenerator.this.serviceName);
				map.put("servicePackage", servicePackage);
				map.put("tenantColumn", MallCodeGenerator.this.tenantColumn);
				this.setMap(map);
			}
		};
		...
		focList.add(new FileOutConfig("/templates/sql/menu.sql.vm") {
			...
			public String outputFile(TableInfo tableInfo) {
				...
				return MallCodeGenerator.this.getOutputDir() + "/sql/" + tableInfo.getEntityName().toLowerCase() + ".menu.mysql";
			}
		});
		focList.add(new FileOutConfig("/templates/entityVO.java.vm") {
			@Override
			public String outputFile(TableInfo tableInfo) {
				return MallCodeGenerator.this.getOutputDir() + "/" + MallCodeGenerator.this.packageName.replace(".", "/") + "/vo/" + tableInfo.getEntityName() + "VO" + ".java";
			}
		});
		focList.add(new FileOutConfig("/templates/entityDTO.java.vm") {
			@Override
			public String outputFile(TableInfo tableInfo) {
				return MallCodeGenerator.this.getOutputDir() + "/" + MallCodeGenerator.this.packageName.replace(".", "/") + "/dto/" + tableInfo.getEntityName() + "DTO" + ".java";
			}
		});
		focList.add(new FileOutConfig("/templates/wrapper.java.vm") {
			@Override
			public String outputFile(TableInfo tableInfo) {
				return MallCodeGenerator.this.getOutputDir() + "/" + MallCodeGenerator.this.packageName.replace(".", "/") + "/wrapper/" + tableInfo.getEntityName() + "Wrapper" + ".java";
			}
		});
		if (Func.isNotBlank(this.packageWebDir)) {
			...
			focList.add(new FileOutConfig("/templates/vue/api.js.vm") {
				@Override
				public String outputFile(TableInfo tableInfo) {
					return MallCodeGenerator.this.getOutputWebDir() + "/api/" + servicePackage.toLowerCase() + "/" + tableInfo.getEntityName().toLowerCase() + ".js";
				}
			});
			focList.add(new FileOutConfig("/templates/vue/crud.vue.vm") {
				@Override
				public String outputFile(TableInfo tableInfo) {
					return MallCodeGenerator.this.getOutputWebDir() + "/views/" + servicePackage.toLowerCase() + "/" + tableInfo.getEntityName().toLowerCase() + ".vue";
				}
			});
	...
	public String getOutputDir() {
		return (Func.isBlank(packageDir) ? System.getProperty("user.dir") + "/mall-common" : packageDir) + "/src/main/java";
	}
